...
export default function AddSpendGatPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      const res = await fetch("/api/spend_gat", {
        method: "POST",
        headers: {
      ...
      if (data.success) {
        ...
        alert(data.message || "فشل إضافة المجموعة");
      }
    } catch (error) {
      console.error(error);
