...
export default function PaymentPage() {
  ...
  const handleToggleReceipt = async (
    ...
    try {
      ...
      if (!data.success) {
        return alert(data.message || "فشل تحديث حالة الاستلام");
      }
              ...
              {payments.map((p) => (
                <tr key={p.pay_id} className="border-t">
                  ...
                  <td className="p-3">
                    <button
                      onClick={() =>
                        handleToggleReceipt(p.pay_id, p.pay_receipt)
                      }
                      className={`px-3 py-1 rounded text-white ${p.pay_receipt
                          ? "bg-green-600"
                          : "bg-gray-500"
                        }`}
                    >
                      {p.pay_receipt ? "مستلم" : "غير مستلم"}
                    </button>
                  </td>
