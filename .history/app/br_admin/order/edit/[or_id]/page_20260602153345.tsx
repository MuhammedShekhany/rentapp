...
export default function EditOrderPage() {
    ...
    const fetchAll = async () => {
      try {
        ...
        if (orderData.success) {
          ...
          setOrPrepareDate(o.or_prepare_date ? o.or_prepare_date.toLocaleDateString('en-CA') : "");
          setOrDateReserve(o.or_date_reserve ? o.or_date_reserve.toLocaleDateString('en-CA') : "");
          setReceiptDate(o.or_date_reciept ? o.or_date_reciept.toLocaleDateString('en-CA') : "");
          console.log(o.or_prepare_date);
  ...
  const addProduct = (product: ProductType, fromModal = false) => {
    setDetails((prev) => {
      // 1. البحث عما إذا كان المنتج موجوداً مسبقاً في القائمة (السلة)
      const existingIndex = prev.findIndex((d) => d.pro_id === String(product.pro_id));

      if (existingIndex > -1) {
        // 2. إذا كان موجوداً: نقوم بتحديث الكمية والمجموع فقط
      ...
      // 3. إذا لم يكن موجوداً: نقوم بإضافته كمنتج جديد
      return [
        ...prev,
        {
      ...
      <div style={s.page} dir="rtl">
        ...
        <div style={s.topBar}>
          <div style={s.topBarInner}>
            ...
            <div style={{ width: 120, height: 34, background: "#e5e7eb", borderRadius: 8 }} />
          </div>
        </div>
        <div style={s.receiptOuter}>
          <div style={s.receipt}>
            ...
            <div style={{ padding: "20px 24px", display: "flex", flexDirection: "column", gap: 14 }}>
              {[180, 140, 100, 220, 160].map((w, i) => (
                <div key={i} style={{ height: 14, width: w, background: "linear-gradient(90deg,#f3f4f6 25%,#e5e7eb 50%,#f3f4f6 75%)", backgroundSize: "200% 100%", borderRadius: 6, animation: "shimmer 1.4s infinite" }} />
              ))}
            </div>
          </div>
        </div>
      ...
      <div style={s.topBar}>
        <div style={s.topBarInner}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            <span style={s.orderIdChip}>تعديل الطلب #{or_id}</span>
          </div>
        ...
        <div style={s.receipt}>
          ...
          {branch?.br_header ? (
            <div style={s.bannerWrap}>
              <img src={branch.br_header} alt={branch.br_name} style={s.bannerImg} />
            </div>
          ) : (
          ...
          <div style={s.metaRow}>
            <div style={s.metaLeft}>
              ...
              <span style={s.metaValue}>{or_id}</span>
            </div>
            ...
            <div
              ...
              style={{
                ...
                userSelect: "none", flexShrink: 0, transition: "0.2s",
                background: or_vip ? "#fef3c7" : "#f3f4f6",
                border: or_vip ? "1px solid #f59e0b" : "1px solid #e5e7eb",
                boxShadow: or_vip ? "0 0 0 2px rgba(245,158,11,0.15)" : "none",
              }}
            >
              <span style={{ fontSize: 15, transition: "0.2s", filter: or_vip ? "drop-shadow(0 0 3px #fbbf24)" : "grayscale(1)", opacity: or_vip ? 1 : 0.5 }}>
                👑
              </span>
              <span style={{ fontSize: 12, fontWeight: 700, whiteSpace: "nowrap", color: or_vip ? "#92400e" : "#6b7280" }}>
                {or_vip ? "VIP" : "عادي"}
              </span>
            </div>
          </div>
          ...
          <div style={{
            ...
            padding: "12px 20px", background: "#fafafa", borderTop: "1px dashed #d1d5db",
          }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
              ...
              <div style={s.cusRow}>
                <div style={{ ...s.cusField, flexDirection: "column", alignItems: "flex-start", gap: 10 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                ...
                <label style={{ fontSize: 11, minWidth: 80, color: "#6b7280", fontWeight: 600 }}>تاريخ الاستلام</label>
                <input
                  type="date" value={receipt_date} readOnly
                  style={{ ...s.extraInput, padding: "4px 6px", height: 30, flex: 1, background: "#f3f4f6" }}
                />
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 2 }}>
                ...
                <label style={{ ...s.extraLabel, fontSize: 11, whiteSpace: "nowrap", minWidth: 80 }}>تاريخ التحضير</label>
                <input
                  type="date" value={or_prepare_date}
          ...
          <div style={s.searchRow}>
            <div style={s.inlineWrap} ref={inlineRef}>
              <svg style={s.searchIcon} width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#9ca3af" strokeWidth="2" strokeLinecap="round">
            ...
            <button onClick={() => setShowModal(true)} style={s.allBtn} className="all-btn">
              📦 كل المنتجات
            </button>
          </div>
                ...
                {details.length === 0 ? (
                  <tr>
                    <td colSpan={5} style={s.emptyRow}>
                      <div style={s.emptyInner}>📦 <span>ابحث وأضف منتجاً</span></div>
                    </td>
                  </tr>
                ) : (
                  details.map((d, i) => (
                    <tr key={d.pro_id} className="trow">
                      ...
                      <td style={{ ...s.td, width: 130 }}>
                        <div style={s.qtyRow}>
                          <button onClick={() => updateQty(i, d.ord_qt - 1)} style={s.qtyBtn}>−</button>
                          <input
                            type="number" min={1} value={d.ord_qt}
                            onChange={(e) => updateQty(i, Number(e.target.value))}
                            style={s.qtyInput} className="rinp"
                          />
                          <button onClick={() => updateQty(i, d.ord_qt + 1)} style={s.qtyBtn}>+</button>
                        </div>
                      </td>
                      <td style={{ ...s.td, textAlign: "center", fontWeight: 700, color: "#111827" }}>
                        {(Number(d.ord_price) * Number(d.ord_qt)).toLocaleString()}
                      </td>
                      <td style={{ ...s.td, width: 40, textAlign: "center" }}>
                        <button onClick={() => removeProduct(d.pro_id)} style={s.delBtn} className="del-btn">✕</button>
                      </td>
                    </tr>
                  ))
                )}
...
// ─────────────────────────────────────────────────
//  STYLES  (identical to AddOrderPage)
// ─────────────────────────────────────────────────
const s: Record<string, React.CSSProperties> = {
  ...
  saveBtn: { display: "flex", alignItems: "center", gap: 6, background: "#0369a1", color: "#fff", border: "none", borderRadius: 8, padding: "8px 18px", fontSize: 13, fontWeight: 700, cursor: "pointer", fontFamily: "inherit", transition: "all 0.18s" },
  orderIdChip: { fontSize: 13, fontWeight: 700, color: "#0369a1", background: "#f0f9ff", border: "1px solid #bae6fd", padding: "4px 12px", borderRadius: 20 },
...
const globalStyles = `
  ...
  * { box-sizing: border-box; }
  ...
  .rinp:focus { border-color: #111827 !important; background: #fff !important; }
  .rinp::placeholder { color: #d1d5db; }

  .drop-item:hover { background: #f9fafb !important; }
  .top-btn:hover { background: #f3f4f6 !important; }
  .save-btn:hover:not(:disabled) { background: #0284c7 !important; }
  ...
  @media (max-width: 600px) { .receipt { border-radius: 0 !important; } }
`;