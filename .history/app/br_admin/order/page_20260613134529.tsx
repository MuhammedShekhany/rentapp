...
export default function OrderPage() {
  ...
  // Initialize state directly from sessionStorage if available to prevent layout shifting
  const [search, setSearch] = useState(() => {
    if (typeof window !== "undefined") {
      return sessionStorage.getItem("order_search_query") || "";
    }
    return "";
  });
  ...
  const [month, setMonth] = useState(() => {
    if (typeof window !== "undefined") {
      return sessionStorage.getItem("order_selected_month") || "";
    }
    return "";
  });
  ...
  useEffect(() => {
    sessionStorage.setItem("order_search_query", search);
  }, [search]);

  useEffect(() => {
    if (month) {
      sessionStorage.setItem("order_selected_month", month);
    }
  }, [month]);
  ...
  useEffect(() => {
    ...
    // Only fallback to current system month if there wasn't a saved session month
    const savedMonth = sessionStorage.getItem("order_selected_month");
    if (!savedMonth) {
      const now = new Date();
  ...
  const totalOrdersAmount = useMemo(() => {
    return orders.reduce((sum, item) => sum + (Number(item.or_total) || 0), 0);
  }, [orders]);
  ...
  const handleDelete = async (or_id: number) => {
    ...
    try {
      const res = await fetch(`/api/order/${or_id}`, { method: "DELETE" });
      const data = await res.json();
  ...
  const toggleDelayed = async (or_id: number, current: number) => {
    try {
      ...
      const res = await fetch(`/api/order/${or_id}`, {
        ...
        body: JSON.stringify({ or_delayed: current == 1 ? 0 : 1 }),
      });
                ...
                <tbody>
                  {filteredOrders.map((item) => (
                    <tr key={item.or_id} className="hover:bg-gray-50">
                      ...
                      <td className="p-4 font-bold text-blue-700">{formatNumber(item.or_total)}</td>
                      <td className="p-4 text-green-700 font-semibold">{formatNumber(item.paid_total)}</td>
                      <td className="p-4 text-red-700 font-semibold">{formatNumber(item.remaining)}</td>
                      <td className="p-4">{item.or_vip == 1 ? "VIP" : "-"}</td>
                      <td className="p-4 whitespace-nowrap">{new Date(item.or_date).toLocaleString("en-GB")}</td>
                      <td className="p-4 whitespace-nowrap">
                        {item.or_date_reserve ? new Date(item.or_date_reserve).toLocaleDateString("en-GB") : "-"}
                      </td>
                      <td className="p-4 whitespace-nowrap">
                        {item.or_date_reserve
                          ? new Date(new Date(item.or_date_reserve).setDate(new Date(item.or_date_reserve).getDate() + 1)).toLocaleDateString("en-GB")
                          : "-"}
                      </td>
                      <td className="p-4 whitespace-nowrap">
                        {item.or_prepare_date ? new Date(item.or_prepare_date).toLocaleDateString("en-GB") : "-"}
                      </td>
                      <td className="p-4">{item.tamin || "-"}</td>
                      <td className="p-4 max-w-[220px]">
                        <div className="truncate">{item.or_note || "-"}</div>
                      </td>
