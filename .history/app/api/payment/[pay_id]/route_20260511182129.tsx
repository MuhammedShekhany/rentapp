...
export async function PUT(req: Request, { params }: ParamsType) {
  try {
    ...
    const body = await req.json();
    const { pay_total, pay_detail, pay_date , pay_receipt } = body;


    const [result]: any = await db.execute(
      `UPDATE payment SET pay_total =COALESCE(?, pay_total) , pay_detail = COALESCE(?, pay_detail) , pay_receipt = COALESCE(?, pay_receipt) , pay_date = COALESCE(?,pay_date)  WHERE pay_id = ?`,
      [Number(pay_total), pay_detail || "", pay_receipt ?? null ,pay_date, pay_id]
    );
