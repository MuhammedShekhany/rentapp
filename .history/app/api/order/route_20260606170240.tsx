  ...
  try {
    ...
    if (month) {
      whereClause += `
        ...
        AND o.or_date_reserve < DATE_ADD(?, INTERVAL 1 MONTH)
      `;
    ...
    const [rows]: any = await db.execute(
      ...
      LEFT JOIN (
        SELECT or_id, SUM(pay_total) AS sum_paid
        FROM payment
        GROUP BY or_id
      ) paid ON paid.or_id = o.or_id

      ${whereClause}
      AND o.or_delayed = 0

      ORDER BY o.or_no DESC, o.or_date_reserve DESC
      `,
      params
    );
...
export async function POST(req: Request) {
  ...
  try {
    ...
    const [orderResult]: any = await conn.execute(
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), (SELECT COALESCE(MAX(o.or_no), 0) + 1 FROM \`order\` o WHERE o.br_id = ?))`,
      [
        or_date,
        Number(or_total || 0),
        or_cus_name || "",
        ...
        br_id,
        user_id,
        br_id // <--- FIXED: Added this here to match the "?" inside the subquery WHERE clause!
      ]
    );
    ...
    // Crucial: Roll back the changes if anything goes wrong
    await conn.rollback();

    console.error("POST order error:", error);

    // If a rare concurrent race condition occurs, catch it safely
    if ((error as any).code === 'ER_DUP_ENTRY') {
      return NextResponse.json({
        success: false,
        message: "Order conflict detected due to simultaneous creation. Please try again."
      }, { status: 409 });
    }
