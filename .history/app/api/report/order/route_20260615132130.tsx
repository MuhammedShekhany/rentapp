...
export async function GET(req: NextRequest) {
  try {
    ...
    // =========================
    // ORDER FILTER (based on order date)
    // =========================
    let where = "WHERE 1=1";
    const params: any[] = [];
    ...
    // =========================
    // PAYMENT FILTER (based on payment date ONLY)
    // =========================
    ...
    if (type === "daily" && date) {
      paymentWhere += " AND DATE(pay_date) = ?";
      paymentParams.push(date);
    }

    if (type === "monthly" && month) {
      paymentWhere += " AND DATE_FORMAT(pay_date,'%Y-%m') = ?";
      paymentParams.push(month);
    }

    if (type === "yearly" && year) {
      paymentWhere += " AND YEAR(pay_date) = ?";
    ...
    const sql = `
      SELECT
        ...
        -- payments only for THIS period (important)
