...
export async function GET(req: NextRequest) {
  try {
    ...
    // =========================
    // PAYMENT FILTER (IMPORTANT FIX)
    // payments are filtered INSIDE SUM (no cross-month leakage)
    // =========================
    const paymentConditions: string[] = [];
    const paymentParams: any[] = [];

    if (br_id) {
      paymentConditions.push("br_id = ?");
      paymentParams.push(br_id);
    }

    if (type === "daily" && date) {
      paymentConditions.push("DATE(pay_date) = ?");
      paymentParams.push(date);
    }

    if (type === "monthly" && month) {
      paymentConditions.push("DATE_FORMAT(pay_date,'%Y-%m') = ?");
      paymentParams.push(month);
    }

    if (type === "yearly" && year) {
      paymentConditions.push("YEAR(pay_date) = ?");
